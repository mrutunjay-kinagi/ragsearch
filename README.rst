...
Retrieval-Augmented Generation (RAG) application that enables natural
language querying over structured data. This tool leverages embedding
models and a vector database (FAISS) to provide an efficient and
scalable search engine.

Features
--------

-  Seamless integration with the Cohere AI LLM for generating
   embeddings.
-  Utilizes FAISS for fast, in-memory vector storage and similarity
   search.
...
Ensure you have your structured data in a CSV, JSON, or Parquet format.
The data should include columns for the main content and any relevant
metadata.
...
Modify the ``llm_model_name`` parameter in ``setup()`` to use different
models, e.g., “large” or “small”.
...
Edit ``index.html`` in the ``templates`` directory to adjust the UI
layout or add more user features.

Troubleshooting
---------------

-  **``AssertionError: d == self.d``**: Ensure the embedding dimension
   (``embedding_dim``) matches the output dimension from your embedding
   model.
-  **``TypeError: embed() takes 1 positional argument``**: Use the
   correct keyword argument format for ``embed()`` based on your
   ``cohere`` version.
...
Feel free to contribute to this project by submitting issues, feature
